...
int main() {
	...
	them_tu(cau.head, "Nguyet");
	them_tu(cau.head, "se");
	them_tu(cau.head, "duoc");
	them_tu(cau.head, "A");
	them_tu(cau.head, "CTDL_GT");
	them_tu(cau.head, "CTDL_GT");
	them_tu(cau.head, "Nguyet");
	them_tu(cau.head, "co");
	them_tu(cau.head, "len");

	cout << "Cau hoan chinh sau khi nhap la: ";
	print_sentence(cau);

	cout << "Tong so tu trong cau la: " << dem_tu_vung(cau.head) << endl;
	cout << "Tu xuat hien nhieu nhat la: \n";
		find_max_app(cau.head);
	
	delete_words(cau.head);
	cout << "Cau hoan chinh sau khi xoa tu lay la: ";
	print_sentence(cau);
